...
def test_clean_tokens():
    ...
    testCase2 = corpysprofiling.clean_tokens(
        corpus, ignore=DEFAULT_PUNCTUATIONS
    )

    assert isinstance(testCase1, list), "Return type is not list"
    assert all(
        isinstance(token, str) for token in testCase1
    ), "Return type is not list of str"
    assert testCase1 == [
        "many",
        "species",
        "animals",
    ...
    assert all(
        isinstance(token, str) for token in testCase2
    ), "Return type is not list of str"
    assert testCase2 == [
        "how",
        "many",
...
def test_corpus_analysis():
    ...
    corpus = (
        "How many species of animals are there in Russia? and how many in US?"
    )
    ...
    ), "Return type is not pd.DataFrame"
    assert testCase1.shape == (
        6,
        1,
    ), "Shape returned does not match output dataframe"
...
def test_corpus_viz():
    ...
    corpus2 = "Let's assume that this unsupervised model is used to assist\
        human experts to identify fraud transaction. So instead of making\
        humans examine all 284,807 transactions for fraud transactions,\
        this model would extract transactions which look suspicious and\
        pass them to humans for examination. So our goal is to come up\
        with a list of transactions which look suspicious.We said before\
        that PCA won't be able to capture characteristic features of fraud\
        transactions because they are like outliers (occur very rarely)\
        in our dataset, and so the reconstruction error would be higher\
        for them compared to non-fraud transactions. But what do we mean\
        by high reconstruction error? What should be threshold which makes\
    ...
    assert isinstance(
        testCase1["word length bar chart"], alt.Chart
    ), "Return type is not an altair Chart"
    ...
    assert isinstance(
        testCase2["word length bar chart"], alt.Chart
    ), "Return type is not an altair Chart"
    ...
    assert isinstance(
        corpysprofiling.corpus_viz(corpus1)["word cloud"], plt.Figure
    ), "Return type is not a word cloud"
    assert isinstance(
        corpysprofiling.corpus_viz(corpus2)["word cloud"], plt.Figure
    ), "Return type is not a word cloud"
...
def test_corpora_compare():
    ...
    testCase2 = corpysprofiling.corpora_compare(
        corpus1, corpus2, metric="euclidean"
    )

    assert isinstance(testCase1, np.float64), "Return type is not np.float"
    assert np.isclose(
        testCase1, 0.0, atol=1e-06
    ), "Identical corpora should return score of 0.0"
    assert np.isclose(
        testCase2, 0.0, atol=1e-06
    ), "Identical corpora should return score of 0.0"
    assert (
        testCase1 >= 0
    ...
    try:
        ...
        assert (
            False
        ), "ValueError not raised. metric must be cosine_similarity\
            or euclidean"
    except ValueError:
...
def test_corpora_best_match():
    ...
    assert testCase1.shape == (
        len(corpora),
        2,
    ), "Shape returned does not match input length of corpora"
    assert Counter(testCase1.corpora.to_list()) == Counter(
        corpora
    ), "Rows in the DataFrame returned do not match elements of corpora"
    assert (
        testCase1.metric.dtype == np.float64
    ), "Distances are not of type np.float64"
    ...
    assert (
        testCase1.metric.is_monotonic_increasing
    ), "Distances should be sorted in ascending order"

    testCase2 = corpysprofiling.corpora_best_match(
        refDoc, corpora, metric="euclidean"
    )

    assert isinstance(
        testCase2, pd.DataFrame
    ), "Return type is not pd.DataFrame"
    assert testCase2.shape == (
        len(corpora),
        2,
    ), "Shape returned does not match input length of corpora"
    assert Counter(testCase2.corpora.to_list()) == Counter(
        corpora
    ), "Rows in the DataFrame returned do not match elements of corpora"
    assert (
        testCase2.metric.dtype == np.float64
    ), "Distances are not of type np.float64"
    # Make sure all distances are greater than or equal to 0
    # (True for euclidean)
    assert (
        testCase2.metric >= 0
    ).all(), "Distances should be greater than or equal to 0 for euclidean"
    # Make sure that distances are sorted in ascending order
    assert (
        testCase2.metric.is_monotonic_increasing
    ), "Distances should be sorted in ascending order"
    ...
    assert testCase3.shape == (
        0,
        2,
    ), "There should be 0 rows when corpora is empty list"
    ...
    try:
        ...
        assert (
            False
        ), "TypeError not raied. corpora_best_match should not accept NoneType\
            inputs"
    except TypeError:
        # TypeError raised as expected
        pass
    ...
    try:
        ...
        assert (
            False
        ), "ValueError not raied. corpora_best_match should not accept NoneType\
            inputs"
    except ValueError:
        # ValueError raised as expected
        pass
